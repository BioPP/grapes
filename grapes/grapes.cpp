...
double lsqrtpi;
double* log_facto;
double positive_dN_corrective_term[(int)VERY_LARGE_S_VALUE+1];
...
void* check_alloc(int nb_blocks, size_t block_size){
  ...
  if(tab==NULL) fin("Not enough memory");
  return tab;
}
...
void printtime(double rtime)
{
	...
	if (d == 0 && h == 0 && m == 0 && s == 0) printf("less than 1 second");
...
void set_constraints(){
  ...
  vector<double> constr_mean; constr_mean.push_back(1.); constr_mean.push_back(pow(10., 20.));
	...
	*y=ya[ns];
	...
	for (m=1;m<n;m++) {
		for (i=1;i<=n-m;i++) {
			...
			hp=xa[i+m]-x;
			w=c[i+1]-d[i];
			if ( (den=ho-hp) == 0.0) fin("Error in routine my_polint"); 
			den=w/den;
			d[i]=hp*den;
			c[i]=ho*den;
		}
...
vector< vector<double> > global_exp_count;
...
double onearg_corrective_fct(double z){ 
  ...
  if(fabs(S)<=QuasiZero) return 1./global_n;
...
void precalculate_dN_corrective_term(){
  ...
  for(double S=-LARGE_S_VALUE-QuasiZero;S<0.;S+=1){
    global_S=S;
    negative_dN_corrective_term[abs((int)round(S))]=qromb(&onearg_corrective_fct, QuasiZero, 1-QuasiZero);
  }


  for(double S=1.-QuasiZero; S<VERY_LARGE_S_VALUE;S+=1){
    global_S=S;
    positive_dN_corrective_term[abs((int)round(S))]=qromb(&onearg_corrective_fct, QuasiZero, 1-QuasiZero);
  }
...
double onearg_S_unfolded_expected_allele_count(double S){
  ...
  // neutral case
  if(fabs(S)<QuasiZero) return global_L*global_theta/global_j;
...
double onearg_scaled_Beta_density(double x){
  ...
  if(x<-SCALED_BETA_BOUNDARY) return 0.;
  if(x>SCALED_BETA_BOUNDARY) return 0.;
...
double onearg_BesselK_density(double x){
  ...
  if(x==0.) return (onearg_BesselK_density(-QuasiZero)+onearg_BesselK_density(QuasiZero))/2.;
...
vector<double> precalculate_integral_S_width(){
  ...
  for(int i=0;i<k;i++){
    if(i==0) width=global_integral_S_point[i+1]-global_integral_S_point[i];
    else if(i==k-1) width=global_integral_S_point[i]-global_integral_S_point[i-1];
    else width=(global_integral_S_point[i+1]-global_integral_S_point[i-1])/2.;
...
double negative_density(string model_name, double S){
  ...
  if(model_name=="ExpoZero") return onearg_negative_Expo_density(S);
  else if(model_name=="ExpoExpo") return onearg_negative_Expo_positive_Expo_density(S);
  else if(model_name=="ExpoGamma") return onearg_negative_Expo_positive_Gamma_density(S);
  else if(model_name=="GammaZero") return onearg_negative_Gamma_density(S);
  else if(model_name=="GammaExpo") return onearg_negative_Gamma_positive_Expo_density(S);
  else if(model_name=="GammaGamma") return onearg_negative_Gamma_positive_Gamma_density(S);
  else if(model_name=="DisplGamma") return onearg_negative_displaced_Gamma_density(S);
  else if(model_name=="ScaledBeta") return onearg_scaled_Beta_density(S);
  else if(model_name=="FGMBesselK") return onearg_BesselK_density(S);
  else return -1.;
}
...
double expected_allele_count_product(double S, int S_rank, int j){
  ...
  double b=global_exp_count[j][S_rank];
...
double expected_allele_count(double L, double theta, map<string,double> param, int n, int j){
  ...
  for(it=param.begin(); it!=param.end(); it++){
    if(it->first=="negGmean") global_Gmean=it->second;
    else if(it->first=="negGshape") global_Gshape=it->second;
    else if(it->first=="posGmean") global_posGmean=it->second;
    else if(it->first=="posGshape") global_posGshape=it->second;
    else if(it->first=="pos_prop") global_pos_prop=it->second;
    else if(it->first=="s0") global_s0=it->second;
    else if(it->first=="Ba") global_Ba=it->second;
    else if(it->first=="Bb") global_Bb=it->second;
    else if(it->first=="med_prop") global_med_prop=it->second;
    else if(it->first=="BKnsz2") global_BKnsz2=it->second;
    else if(it->first=="BKm") global_BKm=it->second;
    else if(it->first=="BKsigma") global_BKsigma=it->second;
    else if(it->first=="BKscale") global_BKscale=it->second;
  } 
...
double S_expected_divergence(double L, double lambda, double theta, int n, double S){
  ...
  //neutral case
  if(fabs(S)<QuasiZero) return L*(lambda+theta/n);

  //selected case
  double dN_correction;
  if(S>0) dN_correction=positive_dN_corrective_term[abs((int)round(S))];
  else dN_correction=negative_dN_corrective_term[abs((int)round(S))];
...
double onearg_S_expected_divergence(double S){
  ...
  //neutral case
  if(fabs(S)<QuasiZero) return global_L*(global_lambda+global_theta/global_n);
...
double expected_divergence_truncated(double L, double lambda, double theta, map<string,double> param, int n, double minS){
  ...
  for(int i=0;i<k;i++){
    if(global_integral_S_point[i]>minS) continue;
    exp_div+=expected_div_product(global_integral_S_point[i])*global_integral_S_width[i];
  }
  ...
  for(unsigned int i=0;i<(*model_list).size();i++){
    for(int j=0;j<fixed_p.nb;j++){
      if((*model_list)[i].name==fixed_p.model_name[j]){
        ...
        if(reparametrization[fixed_p.param_name[j]]=="log")(*model_list)[i].fixed_value[fixed_p.param_name[j]]=log(fixed_p.value[j]);
        else (*model_list)[i].fixed_value[fixed_p.param_name[j]]=fixed_p.value[j];
...
map<string,double> random_starting_point(struct model* m){
  ...
  for(unsigned int i=0;i<param.size();i++){
    ...
    if(reparametrization[param[i]]=="none")
      ...
      if(c_down==0.) c_down=QuasiZero;    
      ran=exp(log(c_down)+dran*(log(c_up)-log(c_down)));
    }

    if(ran>=c_up) ran=c_up-QuasiZero;
    if(ran<=c_down) ran=c_down+QuasiZero;
...
double minus_SFS_loglikelihood(struct onelocus_data data, vector<double> exp_pS, vector<double> exp_pN, double exp_dS, double exp_dN){
  ...
  if(opt.use_poisson){ //calculate likelihood (poisson)
    ...
    if(l!=0) lnL+=k*log(l)-log_facto[k]-l;
    l=exp_dN;
    ...
    l=exp_dS/totexp; 
    k=(int)round(data.fixS);
    if(l!=0) lnL+=k*log(l)-log_facto[k];
    l=exp_dN/totexp; 
    k=(int)round(data.fixN);
    if(l!=0) lnL+=k*log(l)-log_facto[k];
  }
...
/* optimize_saturated */
double optimize_saturated(struct onelocus_data* data){
...
double optimize_neutral(struct onelocus_data* data, double* MLE_oer, double* MLE_neut, struct SFS_div* expectations){
  ...
  if(MLE_neut) *MLE_neut=f*tot_syn_sites/tot_nsyn_sites;
  ...
  if(data->folded==true || !opt.use_syno_orientation_error){
    ...
    for(int i=1;i<=nbval;i++){
      ...
      if(L<maxL){maxL=L; best_oer=oer;}
    }
    if(MLE_oer) *MLE_oer=best_oer;
    if(expectations){
      expectations->divS=exp_dS; expectations->divN=exp_dN;
      expectations->specS=exp_specS; expectations->specN=exp_specN;
    }
...
struct SFS_div expected_SFS_div(struct onelocus_data data, map<string,double> param, double* the, vector<double>* rj){
  ...
  for(int j=1;j<=nc; j++){
    exp_specN[j]=expected_allele_count(data.Ln_poly, 1., param, ng, j);  
  }
...
double minus_loglikelihood(struct onelocus_data data, map<string,double> param, double* the, vector<double>* rj){

  struct SFS_div expected=expected_SFS_div(data, param, the, rj);

  return minus_SFS_loglikelihood(data, expected.specS, expected.specN, expected.divS, expected.divN);
...
class bpp_SFS_minus_lnl : public virtual Function, public AbstractParametrizable{
  private:
    struct onelocus_data* data;
    double lnL;
...
double SFS_max_likelihood(bpp_SFS_minus_lnl lnL_fct, map<string, double> p_init, ParameterList* pl){
  ...
  vector<string> plnames=current_model.param_name;
  for(unsigned int i=0;i<plnames.size();i++){
    if(current_model.optimized[plnames[i]]==false) continue;
    double in;
    if(current_model.reparametrization[i]=="log") in=log(p_init[plnames[i]]); else in=p_init[plnames[i]];
...
vector< map<string,double> > set_initial_values(struct model* m){
  ...
  if(current_DFEM_fit=="ExpoZero"){
    ...
    v_init_p.push_back(basic); v_init_p[0]["negGmean"]=1000.; v_init_p[0]["negGshape"]=1.;
    ...
    v_init_p.push_back(basic); v_init_p[0]["negGmean=1000."]; v_init_p[0]["negGshape"]=1.; v_init_p[0]["posGmean"]=1.;
    v_init_p.push_back(basic); v_init_p[1]["negGmean=100000."]; v_init_p[1]["negGshape"]=0.5; v_init_p[1]["posGmean"]=10.;
    v_init_p.push_back(basic); v_init_p[2]["negGmean=10000000."]; v_init_p[2]["negGshape"]=0.2; v_init_p[2]["posGmean"]=100.;
    v_init_p.push_back(basic); v_init_p[3]["negGmean=10000000."]; v_init_p[3]["negGshape"]=0.2; v_init_p[3]["posGmean"]=1000.; v_init_p[2]["pos_prop"]=0.001;
  }
...
void optimize_DFEM(struct onelocus_data* data, struct model* m, struct parameter_point* optimum){
...
cout <<v_init_p.size() <<" starting points" <<endl;
  ...
  for(unsigned int i=0;i< v_init_p.size();i++){
      ...
      if(lnL>maxlnL && lnL<0.){ maxlnL=lnL; opt_pl=pl;}
  }
...
//cout <<"DFEM_mass:" <<b1 <<" " <<b2 <<endl;
  ...
  if(b1<0. && b2>0.) return DFEM_mass(b1, 0., param)+DFEM_mass(0., b2, param);

  if(param.model_name=="ExpoZero"){
    double sh=1.;
    double m=param.DFEM_param["negGmean"];
    double gsg=gsl_sf_gamma(sh);
    if(b1>b2 && b2<0.) return gsl_sf_gamma_inc(sh, -b2*sh/m)/gsg;
    else if(b1>b2 && b2>=0.) return 0.;
    else if(b1>=0) return 0.;
    else return (gsl_sf_gamma_inc(sh, -b2*sh/m)-gsl_sf_gamma_inc(sh, -b1*sh/m))/gsg;
  }
  ...
  if(param.model_name=="ExpoExpo"){
    ...
    if(b1>b2 && b2<0.) return (1-pp)*gsl_sf_gamma_inc(sh, -b2*sh/m)/gsg_neg;
    else if(b1>b2 && b2>=0.) return pp*gsl_sf_gamma_inc(psh, b1*psh/pm)/gsg_pos;
    else if(b1>=0) return pp*(gsl_sf_gamma_inc(psh, b1*psh/pm)-gsl_sf_gamma_inc(psh, b2*psh/pm))/gsg_pos;
    else return (1-pp)*((gsl_sf_gamma_inc(sh, -b2*sh/m)-gsl_sf_gamma_inc(sh, -b1*sh/m))/gsg_neg);
  }
  ...
  if(param.model_name=="ScaledBeta"){
    ...
    if(b1>b2 && b1<=SCALED_BETA_BOUNDARY) return 1-mp;
    if(b1>b2 && b2>=SCALED_BETA_BOUNDARY) return 0.;
    if(b1>b2 && b2<0.) return DFEM_mass(-SCALED_BETA_BOUNDARY, b2, param);
    if(b1>b2 && b2>=0.) return DFEM_mass(b1, SCALED_BETA_BOUNDARY, param);
    if(b1<-SCALED_BETA_BOUNDARY && b2>SCALED_BETA_BOUNDARY) return 1.;
    if(b1<-SCALED_BETA_BOUNDARY) return DFEM_mass(-SCALED_BETA_BOUNDARY, b2, param);
    if(b2>SCALED_BETA_BOUNDARY) return DFEM_mass(b1, SCALED_BETA_BOUNDARY, param);;

    double scb1=(b1/(2*SCALED_BETA_BOUNDARY))+0.5;
    double scb2=(b2/(2*SCALED_BETA_BOUNDARY))+0.5;
    if(b1>b2 && b2<0.) return mp*gsl_cdf_beta_P(scb2, Ba, Bb);
    else if(b1>b2 && b2>=0.) return mp*gsl_cdf_beta_Q(scb1, Ba, Bb);
    else return mp*(gsl_cdf_beta_P(scb2, Ba, Bb)-gsl_cdf_beta_P(scb1, Ba, Bb));
  }
  ...
  if(param.model_name=="FGMBesselK"){ //numerical discretization for this one
    ...
    if(b2>b1){
      ...
      for(int i=0;i<kk;i++){
        if(global_integral_S_point[i]<b1) continue;
        if(global_integral_S_point[i]>b2) continue;
...
vector<double> calculate_alpha_fww(struct onelocus_data* data, double alfreq_thresh){
 ...
 double tot_poly_N=0., tot_poly_S=0.;
  for(int j=1;j<=global_nbcat;j++){
    if((double)j/(double)global_n<alfreq_thresh) continue;
    tot_poly_N+=data->specN[j];
    tot_poly_S+=data->specS[j];
  } 
  double alpha=1.-(double)(tot_poly_N*data->fixS)/(double)(tot_poly_S*data->fixN);
...
void calculate_alpha_CI(struct onelocus_data* data, struct parameter_point* optimal, double threshold){
  ...
  if(opt.use_divergence_parameter){
    ...
    while(1){
      ...
      if(cur_diff>=threshold && prev_diff<=threshold) break;
      if(cur_divN<opt_divN/5.){problem=true; break;}
      prev_diff=cur_diff;
    }
    if(!problem) optimal->alpha_down=(cur_divN-exp_div_N3)/data->fixN;    
    ...
    while(1){
      ...
      double cur_diff=opt_lnL-cur_lnL;
      if(cur_diff>=threshold && prev_diff<=threshold) break;
      if(cur_divN>opt_divN*3.) {problem=true; break;}
      prev_diff=cur_diff;
    }
    if(!problem) optimal->alpha_up=(cur_divN-exp_div_N3)/data->fixN;    
...
double calculate_alpha_DFEM(struct onelocus_data* data, struct parameter_point* optimal){
  ...
  if(opt.use_divergence_parameter) totN=data->fixN;
  else totN=expected_divergence(data->Ln_div, opt_lambda, optimal->theta, optimal->DFEM_param, data->nb_gene);
...
//cout <<"nbcat:" <<nbcat <<endl;
  ...
  if(opt.use_divergence_parameter) nbparam_neut++;
  if(opt.use_syno_orientation_error) nbparam_neut++;
  printf("  Neutral:\t%.2f\t[%d parameters]\n", neut_lnL, nbparam_neut);
  for(unsigned int i=0;i<v_optimum.size();i++){
    int nbparam=v_model[i].param_name.size()+nbcat+1;
    if(opt.use_divergence_parameter) nbparam++;
  ...
  cout <<"  " <<"Neutral:" <<endl;
  if(opt.use_syno_orientation_error) cout <<"    syno_orientation_error:" <<neut_oer <<endl;
  ...
  for(unsigned int i=0;i<v_optimum.size();i++){
    ...
    for(int j=1;j<nb_bound;j++){
      ...
      if(v_optimum[i].discr_bound[j]==0. || (j>1 && v_optimum[i].discr_bound[j-1]>0 && v_optimum[i].discr_bound[j-2]<0)) cout <<endl <<"    ";
    }
...
void file_output(struct onelocus_data* data, vector<struct parameter_point> v_optimum, vector<struct model> v_model, double sat_lnL, double neut_lnL, double neut_oer, double neut_neut, vector<double> alpha_basic, vector<double> alpha_FWW, FILE* out){
  ...
  if(opt.use_syno_orientation_error) fprintf(out, "Neutral:syno_orientation_error,");
  for(unsigned int i=0;i<implemented_model_names.size();i++){
  ...
  if(opt.use_syno_orientation_error) fprintf(out, "%.4f,", neut_oer);
  for(unsigned int i=0;i<implemented_model_names.size();i++){
    string model_name=implemented_model_names[i];
    for(unsigned int j=0;j<mod[i].param_name.size();j++)
      fprintf(out, "NA,");
  ...
  //other
  for(unsigned int k=0;k<v_optimum.size();k++){
    ...
    fprintf(out, "NA,");
    if(opt.use_syno_orientation_error) fprintf(out, "NA,");
    for(unsigned int i=0;i<implemented_model_names.size();i++){
      string model_name=implemented_model_names[i];
      for(unsigned int j=0;j<mod[i].param_name.size();j++)
...
// pass 1 : folded/unfolded, count loci
  ...
  onelocus_data* dataset=(onelocus_data*)calloc(nbl, sizeof(struct onelocus_data));
...
//pass 2 : read data
  ...
  while(fgets(line, MAXLLINE, in)){
    ...
    if(dataset[nbl].nb_gene!=dataset[0].nb_gene){printf("Same number of genes per locus requested\n"); exit(0);}
...
void read_fixed_param_file(char* fname, struct options* opt){
  ...
  if(f==NULL) {printf("Can't read file: %s\n", fname); exit(0);}
  ...
  while(fgets(line, MAXLLINE, f)){
    ...
    double val;
    if(sscanf(s3, "%le", &val)!=1) continue;
...
void  get_options(int argc, char** argv, struct options* opt, string* infile_name, string* outfile_name){
  ...
  for (int i=1; i<argc ; i++){
    ...
    if(argument=="-in" && i!=argc-1) (*infile_name)=argv[i+1];
    if(argument=="-out" && i!=argc-1) (*outfile_name)=argv[i+1];
  
    if(argument=="-model" && i!=argc-1){
      argument1=argv[i+1];
      for(unsigned int j=0;j<implemented_model_names.size();j++){
        if(argument1==implemented_model_names[j] || argument1=="all")
          opt->do_model[j]=true;
      }
    }

    if(argument=="-no_div_param") opt->use_divergence_parameter=false;
    if(argument=="-no_syn_orient_error") opt->use_syno_orientation_error=false;
    if(argument=="-multinomial") opt->use_poisson=false;
    if(argument=="-nearly_neutral_threshold" && i!=argc-1) sscanf(argv[i+1], "%le", &(opt->nearly_neutral_threshold));
    if(argument=="-FWW_threshold" && i!=argc-1) sscanf(argv[i+1], "%le", &(opt->FWW_threshold));
    if(argument=="-nb_rand_start" && i!=argc-1) sscanf(argv[i+1], "%d", &(opt->nb_random_start));
...
bool check_options(struct options* opt, string infile_name, string outfile_name){
  ...
  for(unsigned int i=0;i<implemented_model_names.size();i++)
    if(opt->do_model[i]) {do_some_model=true; break;}
  if(do_some_model==false) warn_message+="Warning: no selected model - only neutral and saturated will be applied\n";


  if(opt->FWW_threshold<0.) err_message+="Errror: negative FWW_threshold\n";
...
int main(int argc, char** argv){
  ...
  bool do_some_model=check_options(&opt, infile_name, outfile_name);
  nb_random_start_values=opt.nb_random_start;

  /* read data file */

  FILE* input=fopen(infile_name.c_str(), "r");  
  if(input==NULL) {printf("Can't read file: %s\n", infile_name.c_str()); exit(0);}
  int nb_loci=0;
  struct onelocus_data* dataset=read_dofe(input, &nb_loci);
  if(dataset==NULL) {printf("Bad input file: %s\n", infile_name.c_str()); exit(0);}

  global_n=dataset[0].nb_gene;
  global_L=dataset[0].Ln_poly;
  global_nbcat=dataset[0].nb_SFScat;
  global_folded=dataset[0].folded;

  cout <<infile_name <<":" <<endl;
  printf("%d data sets found, %d genes per data set, ", nb_loci, global_n);
  if(global_folded) printf("folded SFS\n"); else printf("unfolded SFS\n");
  if(global_folded) opt.use_syno_orientation_error=false;
  ...
  //TB's corrective term for dN (observing a substitution when the site is actually not fixed)
  if(do_some_model)
    precalculate_dN_corrective_term();
  ...
  vector<double> zerov;
  global_exp_count.push_back(zerov);

  if(do_some_model)
    for(int j=1;j<=global_nbcat;j++)
      global_exp_count.push_back(precalculate_expected_allele_counts(j));

  prov_exp_specS.resize(global_nbcat+1, 0);
  exp_specS.resize(global_nbcat+1, 0);
  exp_specN.resize(global_nbcat+1, 0);
  ...
  //saturated
  double saturated_maxlnL=-optimize_saturated(&dataset[0]);

  //neutral
  double neutral_oer, neutral_neut;
  double neutral_maxlnL=-optimize_neutral(&dataset[0], &neutral_oer, &neutral_neut, NULL);
  ...
  for(unsigned int i=0;i<model_list.size();i++){
    ...
    cout <<model_list[i].name <<"..." <<endl;
    struct parameter_point opt_prov=create_parameter_point(model_list[i].name);
    optimize_DFEM(&dataset[0], &model_list[i], &opt_prov);    
    cout <<"max lnL:" <<opt_prov.lnL <<endl;
  ...
  for(unsigned int i=0;i<v_optimal.size();i++){
    ...
    v_optimal[i].discr_mass.push_back(DFEM_mass(v_optimal[i].discr_bound[nb_bound-1], v_optimal[i].discr_bound[nb_bound-1]-1., v_optimal[i]));    
  }
  ...
  //console
  console_output(v_optimal, model_list, saturated_maxlnL, neutral_maxlnL, neutral_oer, neutral_neut, alpha_basic, alpha_FWW, global_nbcat);
  ...
  if(outfile_name.size()>0){
    ...
    if(output==NULL) fin("Cannot write files");

    file_output(&(dataset[0]), v_optimal, model_list, saturated_maxlnL, neutral_maxlnL, neutral_oer, neutral_neut, alpha_basic, alpha_FWW, output);
  }  
