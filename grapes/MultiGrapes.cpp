...
double negative_density(string model_name, double S){
  ...
  else if(model_name=="ReflectedGamma") return onearg_reflected_Gamma_density(S);
  else return -1.;
}
...
double S_expected_divergence(double L, double lambda, double theta, int n, double S){
  ...
  if(S>0) dN_correction=positive_dN_corrective_term[global_current_species][abs((int)round(S))];
  else dN_correction=negative_dN_corrective_term[global_current_species][abs((int)round(S))];
...
double expected_divergence(double L, double lambda, double theta, map<string,double> param, int n){
  ...
  for(it=param.begin(); it!=param.end(); it++){
    if(it->first=="negGmean") global_Gmean=it->second*ancient_to_recent_Ne_ratio;
    else if(it->first=="negGshape") global_Gshape=it->second;
    else if(it->first=="posGmean") global_posGmean=it->second*ancient_to_recent_Ne_ratio;
    else if(it->first=="posGshape") global_posGshape=it->second;
    else if(it->first=="pos_prop") global_pos_prop=it->second;
    else if(it->first=="s0") global_s0=it->second*ancient_to_recent_Ne_ratio;
    else if(it->first=="Ba") global_Ba=it->second;
    else if(it->first=="Bb") global_Bb=it->second;
    else if(it->first=="med_prop") global_med_prop=it->second/ancient_to_recent_Ne_ratio;
    else if(it->first=="BKnsz2") global_BKnsz2=it->second;
    else if(it->first=="BKm") global_BKm=it->second;
    else if(it->first=="BKsigma") global_BKsigma=it->second;
    else if(it->first=="BKscale") global_BKscale=it->second*=ancient_to_recent_Ne_ratio; 
  } 
...
double SFS_max_likelihood(bpp_SFS_minus_lnl lnL_fct, map<string, double> p_init, ParameterList* pl){
  ...
  try{
    ...
    ml=optim->optimize();
  }catch(Exception & e){ cout << "ERROR! " << e.what() << endl; return 1.;}
...
vector< map<string,double> > set_initial_values(struct model* m){
  ...
  if(!m->optimized["negGshape"]){
    ...
    if(m->fixed_value["negGshape"]<0.12) adjusted_negGmean=1000000.;
    else if(m->fixed_value["negGshape"]<0.2) adjusted_negGmean=100000.;
    else if(m->fixed_value["negGshape"]<0.35) adjusted_negGmean=10000.;
    else if(m->fixed_value["negGshape"]<0.5) adjusted_negGmean=1000.;
    else adjusted_negGmean=100.;
    v_init_p.clear();
...
vector<double> read_fixed_shapes(char* s){
  ...
  prov=strtok(s, ",");
  if(!prov) fin("error -shared_shape option: comma-separated floats expected");

  while(prov){
    ret=sscanf(prov, "%le", &fi);
    if(ret!=1) fin("error -shared_shape option: comma-separated floats expected");
    vfi.push_back(fi);
    prov=strtok(NULL, ",");
  }  
...
void  get_options(int argc, char** argv, struct options* opt, string* infile_name, string* outfile_name){
  ...
  for (int i=1; i<argc ; i++){
    ...
    if(argument=="-no_div_data") opt->use_divergence_data=false;
    ...
    if(argument=="-anc_to_rec_Ne_ratio" && i!=argc-1) sscanf(argv[i+1], "%le", &ancient_to_recent_Ne_ratio);
    if(argument=="-no_separate") opt->do_separate=false; 
    if(argument=="-no_shared") opt->do_shared=false; 
    if(argument=="-shared_shape" && i!=argc-1) opt->fixed_shared_negGshape=read_fixed_shapes(argv[i+1]);
    if(argument=="-fold") opt->fold=true;
    if(argument=="-p_lethal" && i!=argc-1) sscanf(argv[i+1], "%le", &opt->p_vdel);
...
void check_options(struct options* opt, string infile_name, string outfile_name){
  ...
  for(unsigned int i=0;i<implemented_model_names.size();i++){
    if(opt->do_model[i]) {do_some_model=true; nb_model++; opt->model_name=implemented_model_names[i];}
  }

  if(do_some_model==false) err_message+="Error: no selected model\n";

  if(nb_model>1) err_message+="Error: a single model should be passed\n";

  if(opt->FWW_threshold<0.) err_message+="Error: negative FWW_threshold\n";

  if(ancient_to_recent_Ne_ratio<=0.)  err_message+="Error: non-positive anc_to_rec_Ne_ratio\n";

  if(opt->use_divergence_data==false) opt->use_divergence_parameter=false;

  if(!(opt->do_separate || opt->do_shared)) err_message+="Error: neither separate nor shared analysis requested\n";
...
double median_negGshape(vector<struct parameter_point> v_opt){
  ...
  for(unsigned int i=0;i<v_opt.size();i++)
  ...
  if(basic.size()%2==1) return sorted[ns2];
  else return (sorted[ns2-1]+sorted[ns2])/2.;
...
class bpp_SFS_minus_lnl_shared : public virtual Function, public AbstractParametrizable{
  private:
    struct MK_data* data;
    int nb_dataset;
    double lnL;
...
vector< struct parameter_point > optimize_DFEM_shared(struct MK_data* data, int nb_dataset, struct shared_parameter_point* shared_pp, double* multi_ml){
  ...
  ParameterList init=lnL_fct.getParameters();
  if(init.hasParameter("negGshape")) init.setParameterValue("negGshape", shared_pp->negGshape);
  if(init.hasParameter("posGshape")) init.setParameterValue("posGshape", shared_pp->posGshape);
  if(init.hasParameter("pos_prop")) init.setParameterValue("pos_prop", shared_pp->pos_prop);
  ...
  //collect results
  if(optim->getNumberOfEvaluations()<OPTIM_MIN_EVAL) {*multi_ml=-1.; return v_opt;}
  ParameterList finit=lnL_fct.getParameters();
  if(finit.hasParameter("negGshape")) shared_pp->negGshape=finit.getParameterValue("negGshape");
  if(finit.hasParameter("posGshape")) shared_pp->posGshape=finit.getParameterValue("posGshape");
  if(finit.hasParameter("pos_prop")) shared_pp->pos_prop=finit.getParameterValue("pos_prop");
...
int main(int argc, char** argv){
  ...
  for(int k=0;k<nb_dataset;k++){
    ...
    if(dataset[k].div_data_available) tot+=(int)round(dataset[k].fixN)+(int)round(dataset[k].fixS);
...
printtime(difftime(t2, t1));
  ...
  if(opt.do_separate){
    ...
    for(int k=0;k<nb_dataset;k++) multi_maxlnL_sep+=v_opt_sep[k].lnL;
  }
  ...
  if(opt.do_shared){
    ...
    if(opt.fixed_shared_negGshape.size()>0){  //fixed shared shape(s)
      ...
      for(unsigned int i=0;i<opt.fixed_shared_negGshape.size();i++){
      ...
      if(opt.do_separate) shared_pp.negGshape=median_negGshape(v_opt_sep);
      else shared_pp.negGshape=DEFAULT_INITIAL_SHARED_SHAPE;
      shared_pp.posGshape=-1.;
      shared_pp.pos_prop=-1.;
  ...
  if(outfile_name.size()){
    ...
    if(opt.do_shared){
      for(unsigned int i=0;i<v_opt_shared.size();i++){
    ...
    for(int k=0;k<nb_dataset;k++){
      ...
      for(int i=0;i<dataset[k].nb_SFScat;i++) fprintf(output,",%f", dataset[k].specS[i+1]);
      for(int i=0;i<max_nbcat-dataset[k].nb_SFScat;i++) fprintf(output,",NA");
      for(int i=0;i<dataset[k].nb_SFScat;i++) fprintf(output,",%f", dataset[k].specN[i+1]);
      for(int i=0;i<max_nbcat-dataset[k].nb_SFScat;i++) fprintf(output,",NA");
      if(opt.do_separate){
        ...
        for(int i=0;i<dataset[k].nb_SFScat;i++) fprintf(output,",%f", expected.specS[i+1]);
        for(int i=0;i<max_nbcat-dataset[k].nb_SFScat;i++) fprintf(output,",NA");
        for(int i=0;i<dataset[k].nb_SFScat;i++) fprintf(output,",%f", expected.specN[i+1]);
        for(int i=0;i<max_nbcat-dataset[k].nb_SFScat;i++) fprintf(output,",NA");
        for(int i=0;i<dataset[k].nb_SFScat;i++) fprintf(output,",%f", v_opt_sep[k].ri[i+1]);
        for(int i=0;i<max_nbcat-dataset[k].nb_SFScat;i++) fprintf(output,",NA");
      }
