...
double S_expected_divergence(double L, double lambda, double theta, int n, double S)
{
  ...
  if (S > 0)
    dN_correction = positive_dN_corrective_term[abs((int)round(S))];
  else
    dN_correction = negative_dN_corrective_term[abs((int)round(S))];
...
void  get_options(int argc, char** argv, struct options* opt, string* infile_name, string* outfile_name)
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    {
      argument1 = argv[i + 1];
      StringTokenizer st(argument1, ",");
      vector<string> models(st.getTokens().begin(), st.getTokens().end());
      for (size_t j = 0; j < implemented_model_names.size(); j++)
      {
        if (VectorTools::contains(models, implemented_model_names[j]) || argument1 == "all")
          opt->do_model[j] = true;
      }
    }
...
int main(int argc, char** argv)
{
  ...
  for (unsigned int i = 0; i < model_list.size(); i++)
  {
    ...
    optimize_DFEM(&dataset[0], &model_list[i], &opt_prov, outfile_name);
